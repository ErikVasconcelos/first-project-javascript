...
cliente ou não, um booleano que diz se o cliente vai pagar a vista ou não)
    ...
    Os descontos funcionam da seguinte forma:
        ...
        function discount (clientName, purchaseValue, firstOrNoBuyBoolean, BuyInCashOrNoBoolean) {

            if (firstOrNoBuyBoolean && BuyInCashOrNoBoolean) {

                if (purchaseValue > 1000) {purchaseValue * 0.3}
                else if (purchaseValue < 1000 || purchaseValue > 500) {purchaseValue * 0.25}
                else {purchaseValue * 0.2}

            }

            if (firstOrNoBuyBoolean && !BuyInCashOrNoBoolean) {

                if (purchaseValue > 1000) {purchaseValue * 0.2}
                else if (purchaseValue < 1000 || purchaseValue > 500) {purchaseValue * 0.15}
                else {purchaseValue * 0.1}
            ...
            if (!firstOrNoBuyBoolean && !BuyInCashOrNoBoolean) {
                ...
                if (purchaseValue > 1000) {purchaseValue * 0.1}
                else if (purchaseValue < 1000 || purchaseValue > 500) {purchaseValue * 0.05}
                else {'Sem desconto'}
